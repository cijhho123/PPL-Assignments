...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isPredProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse, 
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp,PredProcExp, Program, StrExp, parseL5Program } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp,makeNeverTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makePredProcTExp, makeStrTExp, makeVoidTExp, isLambdaTExp, isPredProcTExp, //addition
         parseTE, unparseTExp, makeUnionTExp, makeInterTExp,    //addition
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType, 
         makeDiffTExp,
         makeAnyTExp, PredProcTExp} from "./TExp";
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    isPredProcExp(exp) ? typeofPredProc(exp, tenv):
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<PredProcTExp> => 
    !isAppExp(e) ? makeFailure("no") :
    !isVarRef(e.rator) ? makeFailure("no") :
    bind (applyTEnv(tenv, e.rator.var), (x: TExp) =>
            isPredProcTExp(x) ? makeOk(x) :
            makeFailure("no"))
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        ...
        () => bind(isTypePredApp(ifExp.test, tenv), (pred: PredProcTExp) => typeofIfWithPredicate(ifExp, pred, tenv)),
        () => typeofIfNormal(ifExp, tenv));


export const typeofIfWithPredicate = (ifExp:IfExp, pred: PredProcTExp, tenv: TEnv): Result<TExp> =>{
    
    if (!isAppExp(ifExp.test) || !isVarRef(ifExp.test.rator))   
        return makeFailure("never");
   
    const inputTest = ifExp.test.rands[0];

    if (!isVarRef(inputTest))
        return makeFailure("unsupported abuse of notation"); //  (if ((lambda (x: any): is? number(number? x)) y) then else)
                                                                //
    const checkedVarName = inputTest.var;

    
    
    return bind(  typeofExp(inputTest,tenv), (ogTExp: TExp) => {
        const testTE = typeofExp(ifExp.test, tenv);
        const thenTE = typeofExp(ifExp.then, makeExtendTEnv([checkedVarName],[pred.typeOfReturn] ,tenv));  //env + x is now type
        
        const newType = makeDiffTExp( ogTExp, pred.typeOfReturn)
        const altTE = typeofExp(ifExp.alt, makeExtendTEnv([checkedVarName], [newType], tenv));  //env + x is NOT type (with diff)
        const constraint1 = bind(testTE, testTE => checkCompatibleType(testTE, makeBoolTExp(), ifExp));
        const constraint2 = bind(thenTE, (thenTE: TExp) =>
            bind(altTE, (altTE: TExp) =>
                 makeOk(makeUnion(thenTE, altTE))));
...
export const typeofPredProc = (tp : PredProcExp, tenv: TEnv): Result<TExp> => {
    const argsTEs = [tp.arg.texp];
    const extTEnv = makeExtendTEnv([tp.arg.var], argsTEs, tenv);
    const constraint1 =  bind(typeofExps(tp.body, extTEnv), (body: TExp) => 
                                checkCompatibleType(body, makeBoolTExp(), tp)); //TODO PAY ATTENTION HERE
    return bind(constraint1, _ => makeOk(makePredProcTExp(argsTEs[0], tp.returnTE)))
}
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        ...
        if (isPredProcTExp(ratorTE)){
            if (app.rands.length !== 1) {
                return bind(unparse(app), (exp: string) => makeFailure<TExp>(`Wrong parameter numbers passed to proc: ${exp}`));
            }
            const constraint = bind(typeofExp(app.rands[0], tenv), (param: TExp) =>
                                                    checkCompatibleType(param,ratorTE.paramTE,app));
            return bind(constraint, _ => makeOk(makeBoolTExp()))
