...
const checkEqualType = (te1: T.TExp | undefined, te2: T.TExp, exp: A.Exp): Result<true> =>
    ...
    // todo? addition
    /*
    T.isAnyTExp(te2)  ? makeOk(true) :
    T.isAnyTExp(te1) ? makeFailure("any type casted to non-any") :
    T.isNeverTExp(te1) ? makeOk(true) :
    T.isNeverTExp(te2) ? makeFailure("non-never casted to never") :   
    */
