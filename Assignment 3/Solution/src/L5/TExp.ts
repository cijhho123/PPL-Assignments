...
;; <atomic-te>    ::= <num-te> | <bool-te> | <void-te> | any | never // addition
...
export type AtomicTExp = NumTExp | BoolTExp | StrTExp | VoidTExp | AnyTExp | NeverTExp; // addition
export const isAtomicTExp = (x: any): x is AtomicTExp =>
    isNumTExp(x) || isBoolTExp(x) || isStrTExp(x) || isVoidTExp(x) || isAnyTExp(x) || isNeverTExp(x);   //addition

export type CompoundTExp = LambdaTExp | TupleTExp | UnionTExp | InterTExp;    //addition
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x) || isInterTExp(x);   //addition

export type NonTupleTExp = AtomicTExp | ProcTExp | TVar | UnionTExp | InterTExp;    //addition
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isProcTExp(x) || isTVar(x) || isUnionTExp(x) || isInterTExp(x);   //addition
...
export const isAnyTExp = (x: any): x is AnyTExp => x.tag === "AnyTExp"; // do we add type substitution here? 
...
//addition
export type LambdaTExp = ProcTExp | PredProcTExp
export const isLambdaTExp = (x:any): x is LambdaTExp => isProcTExp(x) || isPredProcTExp(x);

export type PredProcTExp = { tag: "PredProcTExp"; paramTE: TExp; typeOfReturn: TExp }
export const makePredProcTExp = (paramTE: TExp,  typeofReturn: TExp): PredProcTExp => 
    ({tag: "PredProcTExp", paramTE: paramTE, typeOfReturn: typeofReturn});
export const isPredProcTExp = (x : any): x is PredProcTExp => x.tag === "PredProcTExp";
...
const removeDuplicatesAndAny = (tes: TExp[]): TExp[] =>
    isEmpty(tes) ? tes :
    containsSubType(tes.slice(1), tes[0]) ? removeDuplicatesAndAny(tes.slice(1)) :
...
// SubType comparator (te1 < te2)    e1: any e2: non any
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    //addition any, never

    isTVar(te2) && !isTVar(te1) ? true : // this is bad TODO 
    (isAnyTExp(te2) ) ? true :
    (isAnyTExp(te1)) ? false :
    (isNeverTExp(te1)) ? true :
    (isNeverTExp(te2)) ? false :
    
    //addition for intersection (does it work if one is intersection and one is union?)
    (isInterTExp(te1) && isInterTExp(te2)) ?  isSubset(te1.components, te2.components):
    isInterTExp(te1) ? containsSubType(te1.components, te2): 
    ...
    false; // TODO: if we cannot compare (complicated inter/sub checks) we can just return false

// addition: return a TExp of the set difference between te1 and te2, or te1\te2   
export const makeDiffTExp = (te1:TExp, te2: TExp): TExp => 
    (isAnyTExp(te1) && !isAnyTExp(te2)) ? te1 :
    isAnyTExp(te2)  ? makeNeverTExp():
    
    (isNeverTExp(te1)) ? te1 :
    isNeverTExp(te2) ? te1 :
    
    (isUnionTExp(te1) && isUnionTExp(te2)) ? makeUnionTExp((te1.components).filter( (component: TExp) => (! containsType(te2.components, component))))  :
    (isUnionTExp(te1) && !isUnionTExp(te2)) ? makeUnionTExp((te1.components).filter( (component: TExp) => (component.tag !== te2.tag))):
    (!isUnionTExp(te1) && isUnionTExp(te2)) ? (containsType(te2.components, te1)) ? makeNeverTExp() : te1 :
    (te1 === te2) ? makeNeverTExp() : 
    te1;
...
// true when te is in tes or there is a subtype of te in tes
export const containsSubType = (tes: TExp[], te: TExp) : boolean =>
    isEmpty(tes) ? false:
    isSubType(tes[0], te) ? true :
    containsSubType(tes.slice(1), te);
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> =>
    ...
    (texps[0] === "inter") ? parseInterTExp(texps) :    //addition
    parseLambdaTExp(texps);
...
const parseInterTExp = (texps: Sexp[]): Result<TExp> =>
    //TODO: do we need to remove duplicate at this stage?
    mapv(mapResult(parseTExp, texps.slice(1)), // needs code review.
        (tes: TExp[]) => makeInterTExp(tes));


const parseLambdaTExp = (texps: Sexp[]): Result<LambdaTExp> => 
    texps.indexOf('is?') === -1 ? parseProcTExp(texps):
    parsePredTExp(texps)


//  any -> is? number
const parsePredTExp = (texps: Sexp[]): Result<PredProcTExp> => {
    const pos = texps.indexOf('is?');
    const pos2 = texps.indexOf('->');
    
    return (pos !== 2) ? makeFailure('is? location in syntax malformed'):
            (pos2 != 1) ? makeFailure('predicate syntax malformed'):
            (texps.length != 4) ? makeFailure('predicate syntax not the right length'):
            bind (parseTExp(texps[0]), (arg: TExp) =>
                mapv (parseTExp(texps[3]), (typeOfReturn : TExp) =>
                    makePredProcTExp(arg, typeOfReturn)))
}
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isPredProcTExp(x) ?    bind (unparseTExp(x.paramTE), (parameTE: string) =>
                                        mapv(unparseTExp(x.typeOfReturn), (typeOfReturn: string) =>
                                            [parameTE, '->', 'is?', typeOfReturn])) :
