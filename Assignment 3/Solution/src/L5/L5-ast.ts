...
export type CompoundExp = AppExp | IfExp | LambdaExp | LetExp | LitExp | LetrecExp | SetExp;
...
//addition
export type LambdaExp = ProcExp | PredProcExp;
export const isLambdaExp = (x : any): x is LambdaExp =>
    isProcExp(x) || isPredProcExp(x);


export type PredProcExp = {tag: "PredProc"; arg: VarDecl, body: CExp[]; returnTE: TExp}
export const isPredProcExp = (x:any): x is PredProcExp => x.tag === "PredProc";
export const makePredProcExp = (arg: VarDecl, body: CExp[], returnTE: TExp): PredProcExp =>
                                    ({tag: "PredProc", arg, body, returnTE})    
...
export type PrimOpKeyword = "+" | "-" | "*" | "/" | ">" | "<" | "=" | "not" | "and" | "or" | "eq?" | "string=?" | 
        "cons" | "car" | "cdr" | "list" | "pair?" | "list?" | "number?" | "boolean?" | "symbol?" | "string?" | "any?" | "never?" | //addition
        "display" | "newline";
const isPrimOpKeyword = (x: string): x is PrimOpKeyword =>
    ["+", "-", "*", "/", ">", "<", "=", "not", "and", "or", 
     "eq?", "string=?", "cons", "car", "cdr", "list", "pair?", "any?", "never?",    //addition
...
export const parseL5SpecialForm = (op: SpecialFormKeyword, params: Sexp[]): Result<CExp> =>
    isNonEmptyList<Sexp>(params) ?
        ...
        op === "lambda" ? parseLambdaExp(first(params), rest(params)) :
...
const parseLambdaExp = (vars: Sexp, rest: Sexp[]): Result<LambdaExp> =>
    rest.indexOf('is?') === -1 ? parseProcExp(vars, rest) :
    parsePredExp(vars, rest)


const parsePredExp = (vars: Sexp, rest: Sexp[]): Result<LambdaExp> => {
    if (isArray(vars) && rest[0] === ":" && rest[1] === 'is?' && rest.length >= 2){
        ...
        return bind(args, (args: VarDecl[]) =>
                    bind (body, (body: CExp[]) =>
                        mapv(returnTE, (returnTE : TExp) =>
                            makePredProcExp(args[0], body, returnTE))))
        
    }
    return makeFailure(`failed to parse TypePred expression with args${format(vars)}`);
}
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    isLambdaExp(e) ? unparseLambdaExp(e) :
...
const unparseLambdaExp = (le: LambdaExp): Result<string> =>
    isProcExp(le) ? unparseProcExp(le) :
    unparsePredProc(le);

// (any -> is? type)
const unparsePredProc = (ppe: PredProcExp): Result<string> =>
    bind(unparseVarDecl(ppe.arg), (vds: string) =>
        bind(unparseReturn(ppe.returnTE), (ret: string) =>
            mapv(unparseLExps(ppe.body), (body: string) =>
            `(${vds} -> is? ${ret} ${body})`)));
