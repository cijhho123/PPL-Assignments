// L3-eval.ts
import { map, zipWith, indexOf, slice } from "ramda";
import { Binding, isCExp, isLetExp, makeAppExp, makeBinding } from "./L3-ast";
import { BoolExp, CExp, Exp, IfExp, LitExp, NumExp,
         PrimOp, ProcExp, Program, StrExp, VarDecl, ClassExp } from "./L3-ast"; //addition
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLitExp, isNumExp,
             isPrimOp, isProcExp, isStrExp, isVarRef, isClassExp} from "./L3-ast"; //addition
...
import { isClosure, makeClosure, Closure, Value, isSymbolSExp} from "./L3-value";
import { isClass, makeClass, Class} from "./L3-value";  //addition
import { isObject, makeObject, Object} from "./L3-value";  //addition
...
const applyClass = (exp: Class, args: Value[], env: Env): Result<Value> => {
    ...
    return makeOk(makeObject(binded_var));
}

const applyObject = (proc: Object, args: Value[], env: Env): Result<Value> => 
    isEmpty(args) ? makeFailure("expected arguments") :
    ...
    bind( L3applicativeEval(proc.methods[indexOf(args[0].val, proc.method_names)].val,env),  (v: Value) =>
                    !isClosure(v) ?  makeFailure("this object has a method which is not a closure"):
                    applyClosure(v, args.slice(1), env))
...
const valueToLitExp = (v: Value): NumExp | BoolExp | StrExp | LitExp | PrimOp | ProcExp | ClassExp => //addition
    isNumber(v) ? makeNumExp(v) :
    isBoolean(v) ? makeBoolExp(v) :
    isString(v) ? makeStrExp(v) :
    isPrimOp(v) ? v :
    isClosure(v) ? makeProcExp(v.params, v.body) :
    isClass(v) ? makeClassExp(v.fields, v.methods): //addition
    makeLitExp(v);
