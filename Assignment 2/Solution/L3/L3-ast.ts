...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | ClassExp; //addition 9
...
export type ClassExp = {tag: "ClassExp"; fields: VarDecl[], methods: Binding[]; }    //addition 3
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isClassExp(x);   //addition 6
...
export const parseL3SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "class" ? 
        isNonEmptyList<Sexp>(params) ? parseClassExp(first(params), second(params)) :     //addition 1
...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    ...
    //methods = methods[0].filter((b: Sexp) => isArray(b) && b.length === 2);

    const varDecls = map(makeVarDecl, fields)

    const vars = map(b => b[0], methods) as string[];
    const valsResult = mapResult(parseL3CExp, map(b => b[1], methods));
    const bindingsResult = mapv(valsResult, (vals: CExp[]) => zipWith(makeBinding, vars, vals));

    /*
    return bind(bindingsResult, (bindings: Binding[]) => 
        mapv(mapResult(parseL3CExp, fields), (body: CExp[]) =>
...
export const unparseL3 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp) ? unparseClassExp(exp): //addition 7
