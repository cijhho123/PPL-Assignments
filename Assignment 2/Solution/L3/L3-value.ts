...
import { isPrimOp, CExp, PrimOp, VarDecl, Binding, ClassExp, isBinding } from './L3-ast';
...
export const makeObject = (methods: Binding[]): Object =>
    ({tag: "Object", methods: methods ,method_names: map((v: Binding) => v.var.var, methods), env : makeEmptyEnv()});

export const makeObjectEnv = (methods: Binding[], env: Env): Object =>
    ({tag: "Object", methods: methods ,method_names: map((v: Binding) => v.var.var, methods), env : env});


/*
export const makeObject = (fields: VarDecl[], methods: Binding[]): Object =>
    ({tag: "Object", fields: fields, methods: methods ,method_names: map((v: Binding) => v.var.var, methods), env : makeEmptyEnv()});

export const makeObjectEnv = (fields: VarDecl[], methods: Binding[], env: Env): Object =>
    ({tag: "Object", fields: fields, methods: methods ,method_names: map((v: Binding) => v.var.var, methods), env : env});
...
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | Class | Object; //addition
...
export const valueToString = (val: Value): string =>
    ...
    isClass(val) ? classToString(val):  //addition
